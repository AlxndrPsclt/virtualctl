...
})({"eR5RE":[function(require,module,exports,__globalThis) {
...
module.bundle.HMR_BUNDLE_ID = "52670cdbbb1568c1";
"use strict";
...
},{}],"2ASmQ":[function(require,module,exports,__globalThis) {
/* *
 *
...
`;
        ...
        showtip(d) {
            ...
            function numformat(s, x) {
                ...
                if (s.indexOf("%s") >= 0) return s.replace("%s", x);
                for(; j < s.length; ++j){
        ...
        processMidiEvent(event) {
            ...
            if (this.listeningToThisMidiController(channel, controlNumber)) {
                if (this.tagName == "WEBAUDIO-SWITCH") switch(this.type){
                    case "toggle":
                    ...
                    case "radio":
                        ...
                        for(let i = 0; i < els.length; ++i)if (els[i] == this) els[i].setValue(1);
...
`;
                ...
                if (this.midilearn && this.id) {
                    if (webAudioControlsWidgetManager && webAudioControlsWidgetManager.midiLearnTable) {
                        ...
                        for(let i = 0; i < ml.length; ++i)if (ml[i].id == this.id) {
                ...
                this.setupImage();
                this.digits = 0;
                if (this.step && this.step < 1) for(let n = this.step; n < 1; n *= 10)++this.digits;
...
<defs>
  ...
  <line id="K" x1="32" y1="25" x2="32" y2="11" stroke-linecap="round" stroke-width="6" stroke="${this.coltab[0]}"/>
                    ...
                    for(let i = 0; i < 101; ++i)svg += `<use href="#B" y="${64 * i}"/><use href="#K" y="${64 * i}" transform="rotate(${(-135 + 270 * i / 101).toFixed(2)},32,${64 * i + 32})"/>`;
                    svg += "</svg>";
            ...
            _setValue(v) {
                ...
                if (this._value != this.oldvalue) {
                    ...
                    if (typeof this.convValue == "number") this.convValue = this.convValue.toFixed(this.digits);
                    this.redraw();
                    this.showtip(0);
                    return 1;
                }
            ...
            pointerdown(ev) {
                ...
                let pointermove = (ev)=>{
                    let e = ev;
                    if (ev.touches) {
                        for(let i = 0; i < ev.touches.length; ++i)if (ev.touches[i].identifier == this.identifier) {
                            e = ev.touches[i];
                            break;
                        }
                    }
                    ...
                    } else this._setValue(this.min + ((this.startVal + (this.max - this.min) * offset / ((e.shiftKey ? 4 : 1) * 128) - this.min) / this.step | 0) * this.step);
                    if (this.fireflag) {
                ...
                let pointerup = (ev)=>{
                    let e = ev;
                    if (ev.touches) for(let i = 0;;){
                        if (ev.changedTouches[i].identifier == this.identifier) break;
...
`;
                ...
                if (!this.hasOwnProperty("knobwidth")) Object.defineProperty(this, "knobwidth", {
                    get: ()=>{
                ...
                if (!this.hasOwnProperty("knobheight")) Object.defineProperty(this, "knobheight", {
                    get: ()=>{
                ...
                if (!this.hasOwnProperty("ditchlength")) Object.defineProperty(this, "ditchlength", {
                    get: ()=>{
            ...
            pointerdown(ev) {
                ...
                let pointermove = (ev)=>{
                    ...
                    if (this.tracking == "abs") {
                        ...
                        let val;
                        if (this.dr == "horz") val = Math.max(0, Math.min(1, (e.pageX - rc.left - window.pageXOffset - this.kw * 0.5) / (this.width - this.kw)));
                        else val = 1 - Math.max(0, Math.min(1, (e.pageY - rc.top - window.pageYOffset - this.kh * 0.5) / (this.height - this.kh)));
                        if (this.log) this._setValue(this.min * Math.pow(this.max / this.min, val));
                        ...
                        if (this.log) {
                            ...
                            if (r > 1) r = 1;
                            this._setValue(this.min * Math.pow(this.max / this.min, r));
                        } else this._setValue(this.min + ((this.startVal + (this.max - this.min) * offset / ((e.shiftKey ? 4 : 1) * this.dlen) - this.min) / this.step | 0) * this.step);
                    }
                    if (this.fireflag) {
                        this.sendEvent("input");
                        this.fireflag = false;
...
`;
                ...
                this.elem.onchange = ()=>{
                    if (!this.currentLink.target.conv || this.currentLink.target.conv && this.rconv) {
                        ...
                        if (this.currentLink) this.currentLink.target.setValue(val, true);
                    }
                };
            }
            disconnectedCallback() {}
...
`;
            ...
            redraw() {
                ...
                for(let i = this.min, j = 0; i <= this.max; ++i)if (this.kf[i % 12] == 0) {
                    let x = this.wwidth * j++ + 1;
                    if (this.dispvalues.indexOf(i) >= 0) rrect(this.ctx, x, 1, this.wwidth - 1, this.height - 2, r, this.coltab[5], this.coltab[6]);
                    else rrect(this.ctx, x, 1, this.wwidth - 1, this.height - 2, r, this.coltab[1], this.coltab[2]);
                }
                r = Math.min(8, this.bwidth * 0.3);
                for(let i = this.min; i < this.max; ++i)if (this.kf[i % 12]) {
                    let x = this.wwidth * this.ko[this.min % 12] + this.bwidth * (i - this.min) + 1;
                    if (this.dispvalues.indexOf(i) >= 0) rrect(this.ctx, x, 1, this.bwidth, h2, r, this.coltab[7], this.coltab[8]);
            ...
            pointerdown(ev) {
                this.cv.focus();
                if (this.enable) ++this.press;
                let pointermove = (ev)=>{
                    if (!this.enable) return;
                    let r = this.getBoundingClientRect();
                    let v = [], p;
                    if (ev.touches) p = ev.targetTouches;
                    else if (this.press) p = [
                        ev
                    ];
                    else p = [];
            ...
            sendevent() {
                let notes = [];
                for(let i = 0, j = this.valuesold.length; i < j; ++i)if (this.values.indexOf(this.valuesold[i]) < 0) notes.push([
                    0,
                    this.valuesold[i]
                ]);
                for(let i = 0, j = this.values.length; i < j; ++i)if (this.valuesold.indexOf(this.values[i]) < 0) notes.push([
                    1,
                    this.values[i]
                ]);
                if (notes.length) {
            ...
            setdispvalues(state, note) {
                ...
                } else if (n >= 0) this.dispvalues.splice(n, 1);
            }
    ...
    class WebAudioControlsWidgetManager {
        constructor(){
            ...
            this.updateWidgets();
            if (opt.preserveMidiLearn) this.midiLearnTable = JSON.parse(localStorage.getItem("WebAudioControlsMidiLearn"));
        ...
        handleMIDIMessage(event) {
            ...
            for (let w of this.listOfWidgets)if (w.processMidiEvent) w.processMidiEvent(event);
        ...
        preserveMidiLearn() {
            ...
            const v = [];
            for (let w of this.listOfWidgets)if (w.id) v.push({
                "id": w.id,
                "cc": w.midiController
            });
...
},{}]},["eR5RE","2ASmQ"], "2ASmQ", "parcelRequire94c2")

//# sourceMappingURL=index.bb1568c1.js.map
